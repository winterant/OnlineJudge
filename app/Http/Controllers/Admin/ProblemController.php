...
class ProblemController extends Controller
{
    ...
    public function update(Request $request, $id = -1)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            $update_ret = DB::table('problems')
                ->where('id', $id)
                ->when(!privilege(Auth::user(), 'admin'), function ($q) {
    ...
    public function upload_data(Request $request)
    {
        ...
        if (!$problem)
            return -2;  // 题目不存在返回-2
        if (!$problem || !privilege(Auth::user(), 'admin.problem.data') && Auth::id() != $problem->creator) //不是超级管理员 && 不是出题人 => 禁止上传数据
            return -1;  // 权限不足直接返回-1
    ...
    public function get_data(Request $request)
    {
        ...
        if (
            !$problem ||
            !privilege(Auth::user(), 'admin.problem.data')

...
/*	Two data types are involved in Unicode file names in Windows:
	...
	The module consists of two levels: a set of static routines
		...
		const Fchar *Code_Point_to_UTF16_sequence(UTF32 cp)
...
Fname2str(const Fchar *fn) {
	...
	while (fn[i]) {
		...
		/* get code point from one or two Fchar chars */
		i += UTF16_sequence_to_Code_Point(&fn[i], &cp);
		if (cp == BAD_Code_Point) goto error;
...
str2Fname(const char *s) {
	...
	static Fchar res[MAX_PATH_NAME_LENGTH];
	Fchar *rp = &res[0];
	int i = 0;
...
Fopen(const Fchar *fn, const char *rb) {	/* stream is still char* */
	Fchar fn_copy[MAX_PATH_NAME_LENGTH];

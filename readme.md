...
|---|---|---|---|---|
|2021.08.21|判题潜在bug：系统调用没有完全禁用，如可以提交python代码攻击服务器。解决方案可以按下面一条| | | |
|2021.08.21|判题改进思路：判题端单独做成一个服务，占用一个端口；web端将待判程序主动发送到判题服务（不同服务器第一次需同时将数据发送过去，发送时注意nginx限制）。也可以让判题服务主动发起判题请求，让web分配给它。判题服务可以做成docker镜像，更加安全。| | | |
|2021.08.20|需求：适配centos| | | |
|2021.08.17|需求：竞赛题目，右侧显示题目列表| | | |
|2021.08.17|Web：Admin，将设置移动到各自的模块去。| | | |
|2021.08.16|Judge：判题安全性，分布式判题(要考虑mysql并发)，web端和judge分别部署，judge配置汇总到`judge`文件夹|---|---|---|
|2021.08.16|Judge：判题安全性，运行用户代码时，降低权限，使其没有写权限；|---|---|---|
|2021.08.16|PHP Bug：样例/测试数据的保存，无法保存首/尾空格|原因：laravel request自动去除了首位空白符| | |
...
|2021.08.08|Web&MySql：将样例改为数据库保存，不再以文件方式保存在磁盘中。旧版本升级后样例如何转移到数据库？中间件判断一下？|记得导入导出需要修改| | |

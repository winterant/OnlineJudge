...
|---|---|---|---|---|
|2021.08.16|Judge：判题安全性，分布式判题(mysql并发)，web端和judge分别部署，judge配置汇总到`judge`文件夹|---|---|---|
|2021.08.16|Judge：判题安全性，运行用户代码时，降低权限，使其没有写权限；|---|---|---|
|2021.08.16|PHP Bug：样例/测试数据的保存，无法保存首/尾空格| | | |
|2021.08.16|Web：榜单和气球页面可以设置气球图标（含颜色）| | | |
|2021.08.16|Web：后台大部分table对左右滑动适配把标题也移动了，另外“操作”按钮换行了。（黑名单输入框对手机太宽）| | | |
|2021.08.08|Web&MySql：将图片、样例改为数据库保存，不再以文件方式保存在磁盘中。旧版本升级后图片、样例如何转移到数据库？中间件判断一下|记得导入导出需要修改| | |

              ...
              @foreach ($notices as $item)
                <tr>
                  <td class="text-left" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
                    <a href="javascript:" onclick="get_notice('{{ route('api.notice.get_notice', $item->id) }}')" data-toggle="modal"
                      class="pl-1 text-black @if ($item->state == 2) font-weight-bold @endif" data-target="#modal_notice">{{ $item->title }}</a>
                    @if ($item->state == 2)
      ...
      <div class="col-sm-6 mb-5">
        <div class="card">
          <div class="card-header pt-2 pb-0" style="border-top: 5px solid #fcc700;">
            <a href="javascript:" class="pull-right" style="color: #838383" onclick="whatisthis('自本周一以来的AC题目数量排行榜。每天更新一次。')">
      ...
      <div class="col-sm-6 mb-5">
        <div class="card">
          <div class="card-header pt-2 pb-0" style="border-top: 5px solid #ff0023;">
            <a href="javascript:" class="pull-right" style="color: #838383" onclick="whatisthis('上周一至上周日七天内AC题目数量排行榜。每周一0点更新。')">
              <i class="fa fa-question-circle-o" aria-hidden="true"></i>
            </a>
    ...
    function get_notice(url) {
      $.get(
        ...
        function(ret) {
          ...
          $("#notice-content").html(ret.data.content + "<div class='text-right mt-3'>" + ret.data.created_at + "</div>")
